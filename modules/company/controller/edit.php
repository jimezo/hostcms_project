...
class Company_Controller_Edit extends Admin_Form_Action_Controller_Type_Edit{
	 ...
	 */	public function setObject($object)	{
		...
		// Contacts
		//$oMainTab->move($this->getField('address')->divAttr(array('class' => 'form-group col-xs-12')),$oMainTabRow3);
	...
	protected function _applyObjectProperty()
	{
		...
		{
			$sAddress = trim(Core_Array::getPost("address#{$oCompany_Directory_Address->id}"));

			if (!empty($sAddress))
			{
				$oDirectory_Address = $oCompany_Directory_Address->Directory_Address;
				$oDirectory_Address
					->directory_address_type_id(intval(Core_Array::getPost("address_type#{$oCompany_Directory_Address->id}", 0)))
					->country(strval(Core_Array::getPost("address_country#{$oCompany_Directory_Address->id}", '')))
					->postcode(intval(Core_Array::getPost("address_postcode#{$oCompany_Directory_Address->id}", 0)))
					->city(strval(Core_Array::getPost("address_city#{$oCompany_Directory_Address->id}", '')))
					->value($sAddress)
					->save();
			}
			else
			{
				...
				Core::factory('Core_Html_Entity_Script')
					...
					->value("$.deleteFormRow($(\"#{$windowId} select[name='address_type#{$oCompany_Directory_Address->id}']\").closest('.row').find('.btn-delete').get(0));")
					->execute();
		...
		{
			$sEmail = trim(Core_Array::getPost("email#{$oCompany_Directory_Email->id}"));

			if (!empty($sEmail))
			{
				$oDirectory_Email = $oCompany_Directory_Email->Directory_Email;
				$oDirectory_Email
					->directory_email_type_id(intval(Core_Array::getPost("email_type#{$oCompany_Directory_Email->id}", 0)))
					->value($sEmail)
					->save();
			}
			else
			{
				...
				Core::factory('Core_Html_Entity_Script')
					->type("text/javascript")
					->value("$.deleteFormRow($(\"#{$windowId} select[name='email_type#{$oCompany_Directory_Email->id}']\").closest('.row').find('.btn-delete111').get(0));")
					->execute();
		...
		{
			$sPhone = trim(Core_Array::getPost("phone#{$oCompany_Directory_Phone->id}"));

			if (!empty($sPhone))
			{
				$oDirectory_Phone = $oCompany_Directory_Phone->Directory_Phone;
				$oDirectory_Phone
					->directory_phone_type_id(intval(Core_Array::getPost("phone_type#{$oCompany_Directory_Phone->id}", 0)))
					->value($sPhone)
					->save();
			}
			else
			{
				...
				Core::factory('Core_Html_Entity_Script')
					->type("text/javascript")
					->value("$.deleteFormRow($(\"#{$windowId} select[name='phone_type#{$oCompany_Directory_Phone->id}']\").closest('.row').find('.btn-delete').get(0));")
					->execute();
				$this->_Admin_Form_Controller->addMessage(ob_get_clean());
		...
		foreach ($aCompany_Directory_Websites as $oCompany_Directory_Website)
		{
			...
			$sWebsite_Address = trim(Core_Array::getPost("website_address#{$oCompany_Directory_Website->id}"));
			...
			if (!empty($sWebsite_Address))
			{
				...
				$oDirectory_Website
					->description(Core_Array::getPost("website_description#{$oCompany_Directory_Website->id}"))
			...
			else
			{
				...
				Core::factory('Core_Html_Entity_Script')
					->type("text/javascript")
					->value("$.deleteFormRow($(\"#{$windowId} input[name='website_address#{$oCompany_Directory_Website->id}']\").closest('.row').find('.btn-delete').get(0));")
					->execute();
