												...
												->html("<script>
													   jQuery(function ($) {
															...
															*/
															...
															// Действия с отделами
															$('.department .control-buttons [data-action]').on('click', function(){
																	var \$this = $(this),
															...
															$('li .department-users .scroll-wrapper').each(function (){
																var \$this = $(this);

																if (\$this.find('li.user').length > 3)
																{
																	\$this.slimscroll({
																		// height: '215px',
																		alwaysVisible: true,
					...
					<div class="dd-handle department"><?php echo htmlspecialchars($oCompany_Department->name)?>
						<div class="control-buttons-wrapper department-top-actions">
							<div class="btn-group control-buttons">
								...
								<a href="javascript:void(0);" class="bordered-gray" data-action="edit" title="<?php echo Core::_('Company_Department.editTitleAction', $oCompany_Department->name); ?>" alt="<?php echo Core::_('Company_Department.editTitleAction', $oCompany_Department->name); ?>">
						...
						$aUsers = $oCompany_Department->getHeads();
								...
								<div class="department-head user databox margin-top-5 no-margin-bottom" data-user-id="<?php echo $oUser->id?>" data-company-post-id="<?php echo count($aUser_Company_Posts) ? $aUser_Company_Posts[0]->id : "0" ?>">
									<div class="databox-left">
										...
										echo !empty($oUser->image)
											? "<img style=\"width:45px; height:45px;\" src=\"" . $oUser->getImageFileHref() . "\" class=\"image-circular\"/>"
											: '<span class="btn btn-lg btn-yellow icon-only white"><i class="fa fa-user"></i></span>';
										?>
									</div>
						...
						// Список сотрудников
						$aUsers = $oCompany_Department->getEmployeesWithoutHeads();
						if (count($aUsers))
						{
							?>
							<!-- collapsed -->
