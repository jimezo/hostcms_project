...
$sAdminFormAction = '/admin/event/status/index.php';

$pageTitle = Core::_('Event_Status.event_statuses_title');
...
$oAdmin_Form_Entity_Menus->add(
	Admin_Form_Entity::factory('Menu')
		->name(Core::_('Event_Status.event_statuses_menu_add'))
		->icon('fa fa-plus')
...
$oAdminFormEntityBreadcrumbs
	->add(
		Admin_Form_Entity::factory('Breadcrumb')
			...
			->href
			(
				$oAdmin_Form_Controller->getAdminLoadHref($sEventPath = '/admin/event/index.php')
			)
			->onclick
			(
				$oAdmin_Form_Controller->getAdminLoadAjax($sEventPath)
			)
	)
	->add(
		Admin_Form_Entity::factory('Breadcrumb')
			->name(Core::_('Event_Status.event_statuses_title'))
			->href(
...
{
	$oEvent_Status_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Event_Status_Controller_Edit', $oAdminFormAction
	);

	// Добавляем контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($oEvent_Status_Controller_Edit);

	// Крошки при редактировании
	$oEvent_Status_Controller_Edit->addEntity($oAdminFormEntityBreadcrumbs);
}
...
$oAdminFormDataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Event_Status')
);
