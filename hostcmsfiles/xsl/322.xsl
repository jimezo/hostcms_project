...
<xsl:stylesheet version="1.0"
						...
						<div class="user-tickets">
							...
							<div class="row margin-bottom-20">
								...
								<div class="col-xs-12 col-md-6 text-align-right">
									<button class="btn btn-primary btn-white-text" onclick="$('#AddMessage').toggle('slow')">Добавить сообщение</button>
								</div>
							</div>
	...
	<xsl:template match="helpdesk_message">
		<div id="{@id}" class="comment helpdesk-message-block">
			...
			<div class="helpdesk-message-info">
				...
				<span class="message-answer" onclick="$('.comment_reply').hide('slow');$('#cr_{@id}').toggle('slow')">Ответить</span>
	...
	<xsl:template name="AddReplyForm">
		...
		<form action="{/url}" name="message_form_0{$message_id}" class="show form" method="post" enctype="multipart/form-data">
			...
			<button type="submit" class="btn btn-primary btn-white-text full-width margin-top-10" name="send_message" value="send_message">Отправить</button>
		</form>
	</xsl:template>
</xsl:stylesheet>