		...
		<xsl:variable name="message">
			<xsl:choose>
				...
				<xsl:when test="/document/is_error_time = 1">
					<div class="alert alert-danger">Ошибка! С момента добавления Вами последнего ответа прошло слишком мало времени!</div>
				</xsl:when>
				<!-- Ошибка. Неверный код подтверждения -->
				<xsl:when test="/document/is_error_capthca = 1">
					<div class="alert alert-danger">Ошибка! Вы неверно ввели число подтверждения отправки ответа!</div>
				</xsl:when>
				<!-- Без ошибок -->
				<xsl:otherwise>
