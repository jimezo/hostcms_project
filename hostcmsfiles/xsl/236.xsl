...
<xsl:template match="/document">
	...
	<xsl:choose>
		...
		<xsl:when test="/document/error_time = 1">
			<div class="alert alert-danger">Ошибка! С момента добавления Вами последнего комментария/ответа прошло слишком мало времени!</div>
		</xsl:when>
		<!-- Ошибка. Неверный код подтверждения -->
		<xsl:when test="/document/error_captcha = 1">
			<div class="alert alert-danger">Ошибка! Вы неверно ввели число подтверждения отправки комментария!</div>
		</xsl:when>
		<!-- Без ошибок -->
		<xsl:otherwise>
