...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="shop_item">
		<h1 class="item_title"><xsl:value-of select="name" /></h1>

		<xsl:for-each select="/shop/messages/message">
			<div class="alert alert-success"><xsl:value-of select="."/></div>
		</xsl:for-each>

		<form action="{/shop/structure/link}{@id}/" method="post" enctype="multipart/form-data" class="form-horizontal validate">
			<div class="comment no-background">
				<div class="form-group">
					<label for="{name}" class="col-sm-3 control-label">Заголовок
						<sup>
							<font color="red">*</font>
						</sup>
					</label>
					<div class="col-sm-9">
					<input size="50" type="text" name="name" value="{name}" class="required form-control" minlength="1" title="Заполните поле Заголовок" />
					</div>
				</div>
				<div class="form-group">
					<label for="{name}" class="col-sm-3 control-label">Цена</label>
					<div class="col-sm-9">
					<input size="15" type="text" name="price" value="{price}" class="form-control"/>
					</div>
				</div>
				<div class="form-group">
					<label for="{name}" class="col-sm-3 control-label">Текст объявления</label>
					<div class="col-sm-9">
					<textarea name="text" cols="50" rows="5" class="form-control"><xsl:value-of select="text" disable-output-escaping="yes" /></textarea>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-xs-9">
						<div class="actions">
							<button class="button btn-cart" type="submit" name="update" value="update">
								<i class="fa fa-refresh bg-color5"></i>
