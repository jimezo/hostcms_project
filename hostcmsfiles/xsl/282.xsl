...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="poll">
		<div class="survey_block">
			<span hostcms:id="{@id}" hostcms:field="name" hostcms:entity="poll">
				<xsl:value-of select="name" />
			</span>

			<div class="poll">
				<form action="{/poll_group/url}poll-{@id}/" method="post">
					...
					<div class="actions">
						<button class="btn btn-primary" title="vote" type="submit" name="vote" value="vote">Ответить</button>
					</div>
				</form>
			</div>
		</div>
		...
		<p>
			<label>
				...
				<xsl:when test="../type = 0">
					<input class="poll_input" type="radio" name="poll_response" id="poll_response_{@id}" value="{@id}">
						<xsl:if test="position() = 1"><xsl:attribute name="checked"></xsl:attribute></xsl:if>
					</input>
				</xsl:when>
				<xsl:otherwise>
					<input class="poll_input" type="checkbox" name="poll_response_{@id}" id="poll_response_{@id}" value="{@id}"></input>
				</xsl:otherwise>
				</xsl:choose>
				<xsl:text> </xsl:text><span class="poll_name" hostcms:id="{@id}" hostcms:field="name" hostcms:entity="poll_response"><xsl:value-of select="name"/></span>
			</label>
		</p>
	</xsl:template>
